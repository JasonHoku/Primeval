...
class NoteManagerComponent extends Component {
  ...
  getData() {
    ...
    try {
      this.state.authVar = axios
        .get(`https://api.Primeval.js.com/site-comments`, {
          headers: {
            "content-type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("jwt")}`,
          },
  ...
  onSubmit = () => {
    ...
    axios
      .post(
        `https://api.Primeval.js.com/site-comments`,
  ...
  render() {
    ...
    mutation DeleteComment {
      deleteSiteComment(input: { where: { id: ${this.state.deleteIDVar}  } }) {
        siteComment{
          id
        }
      }
    ...
    return (
      <Fragment>
        <CardHeader>  Site Comment Manager</CardHeader>
        <CardBody>
          <div
            style={{
