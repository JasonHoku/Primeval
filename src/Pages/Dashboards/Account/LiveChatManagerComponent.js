...
const apolloClient = new ApolloClient({
  ...
  link: new HttpLink({
    uri: "https://api.Primeval.js.com/graphql",
    headers: {
...
class NoteManagerComponent extends Component {
  ...
  getData() {
    ...
    try {
      this.state.authVar = axios
        .get(`https://api.Primeval.js.com/live-chats`, {
          headers: {
            "content-type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("jwt")}`,
          },
  ...
  purgeOldChats() {
    ...
    for (var i = 0; i < this.state.getOldTimestamp.length; i++) {
      ...
      if (new Date(this.state.getOldTimestamp[i]) < myStartDate) {
        axios
          .delete(
            `https://api.Primeval.js.com/live-chats/${this.state.getDataEZID[i]}`,
  ...
  purgeByID = () => {
    axios
      .delete(
        `https://api.Primeval.js.com/live-chats/${this.state.deleteIDVar}`,
  ...
  onSubmitDelete = () => {
    ...
    axios
      .post(
        `https://api.Primeval.js.com/live-chats`,
